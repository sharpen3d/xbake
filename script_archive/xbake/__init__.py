        ...
        if allowBake:
            ...
            if obj:
                ...
                if src:
                    ...
                    # Split the names by valid delimiters and find the common prefix
                    source_parts = re.split(r'[_\-.]', source_name)
                    target_parts = re.split(r'[_\-.]', name)
                    
                    # Find the common prefix from the split parts
                    common_parts = []
                    for src_part, tgt_part in zip(source_parts, target_parts):
                        if src_part == tgt_part:
                            common_parts.append(src_part)
                        else:
                            break
                    
                    # Join the common parts with a valid delimiter, if any
                    if common_parts:
                        common_name = '_'.join(common_parts)
                    else:
                        common_name = ''  # No common name found

                    # Only use the common name if it is non-empty
                    if common_name:
                        name = re.sub(r'[^a-zA-Z0-9]+$', '', common_name)  # Strip any trailing non-alphanumeric characters
